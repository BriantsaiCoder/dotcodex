<!-- GENERATED by ~/.agents/bin/agents-sync @e3245b3 hash:2ef68692 — 勿手改；改 ~/.agents/core|hosts|rules -->
<!-- FP:AGENTS-T0-2026Q3 -->
...
[T0-3] MUST NOT force-push main/master；非保護分支只用 --force-with-lease。觸發：git push --force* 且目標為 main|master。例外：無。驗證：Claude=guard-git-push.sh PreToolUse 前置攔截（async:false 可 block）+ audit-bash 事後稽核 log（async，僅記錄不攔截）；Copilot=~/.copilot/hooks/guard-git-push.json preToolUse deny 攔截（2026-07-13 CLI 1.0.70 端對端驗證：deny 探針回 "Denied by preToolUse hook"）；Codex=~/.codex/hooks.json PreToolUse(Bash) guard + ~/.codex/rules/default.rules forbidden defense-in-depth（2026-07-14 contract / execpolicy canary 12/0；PreToolUse 對 unified_exec 覆蓋不完整，仍須 prose + repo pre-commit/CI 疊加）。
...
- [R-3] S2 ⏸ plan gate 在任何 auto / autopilot 模式下 MUST NOT 豁免。觸發：非 plan 模式且將改檔。驗證：改檔前有 session 內 plan artifact（host plan/todo 或對話中的完整計畫）＋引用用戶確認原句；repo 內 plan file 僅在核准後按需持久化，不作為進入實作的必要條件。例外：無。
