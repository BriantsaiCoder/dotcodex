<!-- GENERATED by ~/.agents/bin/agents-sync @3035553 hash:aa058329 — 勿手改；改 ~/.agents/core|hosts|rules -->
<!-- FP:AGENTS-T0-2026Q3 -->
...
[T0-3] MUST NOT force-push main/master；非保護分支只用 --force-with-lease。觸發：git push --force* 且目標為 main|master。例外：無。驗證：Claude=guard-git-push.sh PreToolUse 前置攔截（async:false 可 block）+ audit-bash 事後稽核 log（async，僅記錄不攔截）；Codex/Copilot=hooks 能力已有（PreToolUse 可 deny）但現況未配置，靠本條 prose + repo 層 pre-commit/CI（勿假設 hook 存在）。
