...
[exposure.json](exposure.json) selects five names for `skills.config` entries with `enabled=false`: the three Codex rescue helpers and Ponytail gain/review. [exposure-evidence.json](exposure-evidence.json) records the native prompt-input observer removing exactly those five entries (92→87), with no unexpected additions. This controls catalog exposure only: keep both plugins enabled, their tools/runtime and all hook handlers installed. Claude configuration is not changed.

After repository gates, merge these five selectors into the existing skills configuration through the native config API, preserving unrelated entries and using the current config version. Re-run the observer and hook metadata probe. Rollback removes only the five entries added by this change, or restores a matching entry's recorded previous value; never replace the full config. Chrome and plugin React remain exposed until their distinct capability canaries pass.
